...
class ImageCaptionDataset(Dataset):
    @staticmethod
    def add_args(parser):
        parser.add_argument("--data_root", type=str, default="/data1/scratch/places_hdf5_pyp")
    ...
    def __init__(self, args, split = "train"):
        ...
        split_name = split.replace("val", "dev") if not self.args.test else split.replace("val", "test")
        audio_dataset_json_file = os.path.join(args.data_root, f"metadata/{split_name}_2020.json")
        img_dataset_h5py_file = os.path.join(args.data_root, f"places_imgfeat/{split_name}_2020.hdf5")
        img_id2index_file = os.path.join(args.data_root, f"metadata/{split_name}_2020_imgid2idex.json")
        imgid2ordered_indices_file = os.path.join(args.data_root, f"metadata/{split_name}_2020_imgid2ordered_indices.pkl")
