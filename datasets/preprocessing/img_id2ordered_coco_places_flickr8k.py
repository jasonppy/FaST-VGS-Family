...
def generate_pkl(audio_dataset_json_file, img_dataset_hdf5, img_id2ordered_file, img_data, places = False, flickr8k=False):
    ...
    for index, img_id in enumerate(all_img_id):
        ...
        # if img_id not in img_id2ordered:
        datum = img_data[img_id]
        ordered_index = np.argsort(datum['objects_conf'])[::-1]
        # ordered_index = np.argsort(datum['attrs_conf'])[::-1]
        # print(ordered_index)
        # print(np.sort(datum['attrs_conf'])[::-1])
        # print()
        ...
        if index % 5000 == 0:
            ...
            # break
    # np.savez(img_id2ordered_file, **img_id2ordered)
...
if __name__ == "__main__":
    ...
    if args.dataset == 'coco':
        ...
        train_imgid2ordered_file = "/data1/scratch/coco_pyp/SpokenCOCO/SpokenCOCO_train_imgid2ordered_indices.pkl"
        val_imgid2ordered_file = "/data1/scratch/coco_pyp/SpokenCOCO/SpokenCOCO_val_imgid2ordered_indices.pkl"
        ...
        generate_pkl(train_audio_dataset_json_file, train_img_dataset_hdf5, train_imgid2ordered_file, img_data)
        generate_pkl(val_audio_dataset_json_file, val_img_dataset_hdf5, val_imgid2ordered_file, img_data)
        ...
        splits = ['train_2020', 'dev_unseen_2020', 'test_seen_2020', 'test_unseen_2020']
        # splits = ['dev_seen_2020']
        for split in splits:
        # train_audio_dataset_json_file="/data1/scratch/coco_pyp/SpokenCOCO/SpokenCOCO_train_unrolled.json"
            ...
            imgid2ordered_file =os.path.join(json_root, split+"_imgid2ordered_indices.pkl") 
            ...
            generate_pkl(audio_dataset_json_file, img_dataset_hdf5, imgid2ordered_file, img_data, places=True)
        ...
        train_imgid2index_file = "/data1/scratch/datasets_pyp/flickr8k/flickr8k_train_imgid2ordered_indices.pkl"
        val_imgid2index_file = "/data1/scratch/datasets_pyp/flickr8k/flickr8k_dev_imgid2ordered_indices.pkl"
        test_imgid2index_file = "/data1/scratch/datasets_pyp/flickr8k/flickr8k_test_imgid2ordered_indices.pkl"
        ...
        for img_datum in img_data_val:
            img_data[img_datum['img_id']] = img_datum
        generate_pkl(val_audio_dataset_json_file, val_img_dataset_hdf5, val_imgid2index_file, img_data, flickr8k=True)

        print("test")
        img_data = {}
        for img_datum in img_data_test:
            img_data[img_datum['img_id']] = img_datum
        generate_pkl(test_audio_dataset_json_file, test_img_dataset_hdf5, test_imgid2index_file, img_data, flickr8k=True)

        print("train")
        img_data = {}
        for img_datum in img_data_train:
            img_data[img_datum['img_id']] = img_datum
        generate_pkl(train_audio_dataset_json_file, train_img_dataset_hdf5, train_imgid2index_file, img_data, flickr8k=True)